            ...
            while self.running:
                try:
                    ...
                    try:
                        ...
                        self.log_message("Could not send error response to client, closing connection")
                        break
    ...
    def _manage_clip_automation(self, track_index, clip_index, action, parameter_name="", params=None):
        ...
        try:
            ...
            if action == "create":
                ...
                if envelope is None:
                    return {"status": "error", "message": "Could not create automation envelope for parameter: " + param.name}
                raw_time = params.get("time_in_beats", 0.0) if isinstance(params, dict) else 0.0
                raw_value = params.get("value", 0.5) if isinstance(params, dict) else 0.5
                time_in_beats = max(0.0, float(raw_time))
                normalized_value = max(0.0, min(1.0, float(raw_value)))
                envelope.insert_step(time_in_beats, 0.0, normalized_value)
                return {
                    "action": "add_point",
